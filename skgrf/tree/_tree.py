...
class Tree:
    ...
    def n_node_samples(self):
        ...
        n_samples = [
            len(node) if node else 1 for node in self.grf_forest["leaf_samples"][0]
        ]
    ...
    def weighted_n_node_samples(self):
        ...
        return np.array([sum(w) if w else 100 for w in weighted_n_samples])
    ...
    def value(self):
        ...
        values = self.grf_forest["leaf_values"][0].copy()
        self._get_values(
            self.children_left,
            self.children_right,
            self._root_node_index,
        ...
        weights = self.grf_forest["leaf_weights"][0].copy()
        self._get_values(
            self.children_left,
            self.children_right,
            self._root_node_index,
