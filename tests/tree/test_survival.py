...
class TestGRFTreeSurvival:
    ...
    def test_fit(self, lung_X, lung_y):
        ...
        assert gfs.grf_forest_["_num_trees"] == 1
    ...
    def test_equalize_cluster_weights(
        ...
        gfs = GRFTreeSurvival(equalize_cluster_weights=equalize_cluster_weights)
    ...
    def test_sample_fraction(self, lung_X, lung_y, sample_fraction):
        gfs = GRFTreeSurvival(sample_fraction=sample_fraction)
    ...
    def test_honesty(self, lung_X, lung_y, honesty):
        gfs = GRFTreeSurvival(honesty=honesty)
        gfs.fit(lung_X, lung_y)
    ...
    def test_honesty_prune_leaves(self, lung_X, lung_y, honesty_prune_leaves):
        gfs = GRFTreeSurvival(honesty=True, honesty_prune_leaves=honesty_prune_leaves)
        gfs.fit(lung_X, lung_y)

    def test_alpha(self, lung_X, lung_y, alpha):
        gfs = GRFTreeSurvival(alpha=alpha)
